...
#
# File: dialnlp/decoders/hgfu_rnn_decoder.py
# Date: 2018/12/23 16:55:30
...
class RNNDecoder(nn.Module):
    ...
    def initialize_state(self,
        ...
        if knowledge_lengths is not None and attn_k_mask is None:
            max_len = knowledge.size(1)
            attn_k_mask = sequence_mask(knowledge_lengths, max_len).eq(0)
    ...
    def decode(self, input, state, is_training=False):
        ...
        if self.attn_mode is not None:
            weighted_knowledge, _ = self.knowledge_attention(query=hidden[-1].unsqueeze(1),
                                                 memory=state.knowledge,
                                                 mask=state.attn_k_mask)
                            
        cue_input_list.append(weighted_knowledge)
